...
Download and unzip the [zip-archive](https://github.com/green-api/telegram-demo-chatbot-golang) of the project or clone it with the version control system command:
...
```
    git clone https://github.com/green-api/telegram-demo-chatbot-golang
```
...
The library [telegram-chatbot-golang](https://github.com/green-api/telegram-chatbot-golang) contains a mechanism for changing instance settings using the [SetSettings](https://green-api.com/telegram/docs/api/account/SetSettings/) method, which is launched when the chatbot is turned on.
...
```
which are responsible for receiving notifications about incoming messages.
...
After the settings are applied, notifications about previously received incoming messages will be deleted. This process is also written in the library [telegram-chatbot-golang](https://github.com/green-api/telegram-chatbot-golang) and starts automatically after changing the settings.
...
5 - עברית"
```
Answer 1 -5  to select the language for further communication. After you send 1, the chatbot will send a welcome message in English:
```
...
The chatbot uses the library [telegram-chatbot-golang](https://github.com/green-api/telegram-chatbot-golang), where methods for sending and receiving messages are already integrated, so messages are read automatically and sending regular text messages is simplified.
...
```
However, other send methods can be called directly from the [telegram-api-client-golang](https://github.com/green-api/telegram-api-client-golang) library. Like, for example, when receiving an avatar:
```go
...
[LICENSE](https://github.com/green-api/telegram-demo-chatbot-golang/blob/master/LICENCE).